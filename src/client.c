/*
 * chat_client.c — Cliente de Chat Multithread con interfaz ncurses
 * CC3064 Sistemas Operativos — Proyecto 01
 * Grupo: José Mérida, Javier España, Angel Esquit
 *
 * Compilar: gcc -o chat_client client.c -lpthread -lncurses -Wall
 * Uso:      ./chat_client <usuario> <IP_servidor> <puerto>
 */
...
#define MAX_NAME    64
#define MAX_LOG     500   /* líneas en el historial de chat */
...
static char   my_status[16] = "ACTIVO";
static int    running   = 1;

/* Chat log */
typedef struct { char text[512]; int color; } LogLine;
...
static void redraw_chat(void) {
    ...
    int visible = rows - 6; /* altura útil del panel */
    if (visible < 1) { pthread_mutex_unlock(&screen_mutex); return; }
...
/* ── Redibujar barra de estado ───────────────────────────────── */
static void redraw_status(void) {
    pthread_mutex_lock(&screen_mutex);
    werase(win_status);
    wattron(win_status, COLOR_PAIR(COL_HEADER));
    mvwprintw(win_status, 0, 1, " Usuario: %-20s Estado: %-10s  "
              "[/help para comandos]", username, my_status);
...
static void init_ui(void) {
    ...
    /* Panel de chat ocupa la mayor parte */
...
static void process_server_msg(const char *raw) {
    ...
    if (strcmp(fields[0], "OK") == 0) {
        snprintf(line, sizeof(line), "✓ %s", nf > 1 ? fields[1] : "OK");
        add_log(line, COL_SERVER);

    } else if (strcmp(fields[0], "ERROR") == 0) {
        snprintf(line, sizeof(line), "✗ Error %s: %s",
                 nf > 1 ? fields[1] : "?",
                 nf > 2 ? fields[2] : "");
        add_log(line, COL_ERROR);

    } else if (strcmp(fields[0], "SERVER_BROADCAST") == 0 && nf >= 3) {
    ...
    } else if (strcmp(fields[0], "SERVER_DIRECT") == 0 && nf >= 3) {
        snprintf(line, sizeof(line), "🔒 [DM de %s] %s", fields[1], fields[2]);
        add_log(line, COL_DM);

    } else if (strcmp(fields[0], "USER_LIST") == 0) {
    ...
    } else if (strcmp(fields[0], "USER_INFO") == 0 && nf >= 4) {
        snprintf(line, sizeof(line), "  %s  IP: %s  Estado: %s",
                 fields[1], fields[2], fields[3]);
        add_log(line, COL_STATUS);

    } else if (strcmp(fields[0], "STATUS_UPDATE") == 0 && nf >= 3) {
        strncpy(my_status, fields[2], sizeof(my_status) - 1);
        snprintf(line, sizeof(line), "✓ Estado cambiado a %s", fields[2]);
        add_log(line, COL_SERVER);
        redraw_status();

    } else if (strcmp(fields[0], "FORCED_STATUS") == 0 && nf >= 3) {
        strncpy(my_status, fields[2], sizeof(my_status) - 1);
        snprintf(line, sizeof(line), "⚠ Estado cambiado a %s por inactividad", fields[2]);
        add_log(line, COL_STATUS);
        redraw_status();

    } else if (strcmp(fields[0], "USER_JOINED") == 0 && nf >= 2) {
        snprintf(line, sizeof(line), "→ %s se conectó", fields[1]);
        add_log(line, COL_SERVER);

    } else if (strcmp(fields[0], "USER_LEFT") == 0 && nf >= 2) {
        snprintf(line, sizeof(line), "← %s se desconectó", fields[1]);
        add_log(line, COL_SERVER);
...
static void *recv_thread(void *arg) {
    ...
    while (running) {
        ...
        if (n <= 0) {
            if (running) {
                add_log("✗ Conexión con el servidor perdida.", COL_ERROR);
                redraw_chat();
                running = 0;
            }
            return NULL;
        ...
        /* Puede recibirse más de un mensaje en un recv */
...
static void process_input(const char *line) {
    ...
    if (strcmp(line, "/list") == 0) {
        snprintf(cmd, sizeof(cmd), "LIST_USERS|%s", username);
        server_send(cmd);
        return;
    }
    ...
    if (strncmp(line, "/info ", 6) == 0) {
        ...
        snprintf(cmd, sizeof(cmd), "GET_USER_INFO|%s|%s", username, target);
        server_send(cmd);
        return;
    }
    ...
    if (strncmp(line, "/status ", 8) == 0) {
        const char *s = line + 8;
        snprintf(cmd, sizeof(cmd), "CHANGE_STATUS|%s|%s", username, s);
        server_send(cmd);
        return;
    }
    ...
    if (strncmp(line, "/dm ", 4) == 0) {
        /* /dm <usuario> <mensaje> */
        char tmp[BUF_SIZE];
        strncpy(tmp, line + 4, sizeof(tmp));
        char *sp = strchr(tmp, ' ');
        if (!sp) { add_log("Uso: /dm <usuario> <mensaje>", COL_ERROR); redraw_chat(); return; }
        *sp = '\0';
        const char *target = tmp;
        const char *msg    = sp + 1;
        snprintf(cmd, sizeof(cmd), "MSG_DIRECT|%s|%s|%s", username, target, msg);
        server_send(cmd);
    ...
    if (strcmp(line, "/exit") == 0) {
        snprintf(cmd, sizeof(cmd), "DISCONNECT|%s", username);
        server_send(cmd);
        running = 0;
        return;
    }
    ...
    snprintf(cmd, sizeof(cmd), "MSG_BROADCAST|%s|%s", username, line);
    server_send(cmd);
}
...
static void input_loop(void) {
    ...
    while (running) {
        ...
        if (ch == ERR) { usleep(10000); continue; }
        ...
        if (ch == '\n' || ch == KEY_ENTER) {
            ...
            if (pos > 0) {
                pos--;
                pthread_mutex_lock(&screen_mutex);
                int cy, cx; getyx(win_input, cy, cx); (void)cy;
                if (cx > 2) { wmove(win_input, 1, cx - 1); wdelch(win_input); }
                wrefresh(win_input);
                pthread_mutex_unlock(&screen_mutex);
            }
...
int main(int argc, char *argv[]) {
    ...
    /* Inicializar UI antes de registrar para mostrar respuesta */
    init_ui();
    redraw_status();
    show_help();
    ...
    /* Loop de input (bloquea hasta /exit o pérdida de conexión) */
